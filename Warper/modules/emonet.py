...
class EmoNet(nn.Module):
    ...
    def encode_feat(self, x, reset_smoothing=False):
        # Resets the temporal smoothing
        if self.init_smoothing:
            self.init_smoothing = False
            self.temporal_state = torch.zeros(x.size(0), self.n_temporal_states, self.n_expression + self.n_reg).cuda()
        ...
        feat = {
            128: None, 64: None, 32: None, 16: None, 8: None,
        }

        x = self.conv1(x)
        feat[128] = x
        x = F.relu(self.bn1(x), True) # (bs, 64, 128, 128)
        x = F.max_pool2d(self.conv2(x), 2, stride=2) # (bs, 128, 64, 64)
        x = self.conv3(x) # (bs, 128, 64, 64)
        feat[64] = x
        x = self.conv4(x) # (bs, 256, 64, 64)
        feat[32] = x
        ...
        emo_feat_conv1D = self.conv1x1_input_emo_2(emo_feat) # (4, 256, 64, 64)
        feat[16] = feat[8] = x
        final_features = self.emo_net_2(emo_feat_conv1D) # (4, 256, 4, 4)
        final_features = self.avg_pool_2(final_features) # (4, 256, 1, 1)
        batch_size = final_features.shape[0]
        final_features = final_features.view(batch_size, final_features.shape[1])

        return final_features, feat
