...
class DenseMotionNetwork(nn.Module):
    ...
    def forward(self, feature, kp_driving, kp_source):
        ...
        deformed_feature = self.compress(deformed_feature)  # (bs*3*(num_kp+1), 16, 256, 256)
        deformed_feature = self.norm(deformed_feature)  # (bs*3*(num_kp+1), 16, 256, 256)
        deformed_feature = F.relu(deformed_feature)  # (bs*3*(num_kp+1), 16, 256, 256)
        ...
        input = input.view(bs*3, -1, h_c, w_c)  # (bs*3, (1+num_kp)*c=22*4, h=256, w=256)
    ...
    def _forward(self, feature, kp_driving, kp_source):
        # TODO: out of mm
        bs, _, _, c, h, w = feature.shape # (bs, 3, 1, 32, 512, 512)
        ...
        out_dict['mask'] = mask
        mask = mask.unsqueeze(2)                                   # (bs * 3, num_kp+1, 1, h, w)
        feature = feature.view(bs * 3, -1, 32, h, w)    # (bs * 3, num_kp+1, 32, h, w)
        deformation = (feature * mask).sum(dim=1)            # (bs * 3, 32, h, w)  mask take effect in this place
        deformation = deformation.permute(0, 2, 3, 1)           # (bs * 3, h, w, 2)
