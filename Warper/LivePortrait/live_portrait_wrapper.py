...
class LivePortraitWrapper(nn.Module):
    """
    Wrapper for Human
    """
    ...
    def prepare_source(self, img: torch.Tensor) -> torch.Tensor:
        """ construct the input as standard
        img: HxWx3, uint8, 256x256
        """
        h, w = img.shape[:2]
        if h != self.inference_cfg.input_shape[0] or w != self.inference_cfg.input_shape[1]:
            x = F.interpolate(img, size=(self.inference_cfg.input_shape[0], self.inference_cfg.input_shape[1]))# (bs, 3, 256, 256)
        else:
            x = img.copy()

        if len(x.shape) == 3:
            x = x.unsqueeze(0).type(torch.float32) / 255.  # HxWx3 -> 1xHxWx3, normalized to 0~1
        elif len(x.shape) == 4:
            x = x.type(torch.float32) / 255.  # BxHxWx3, normalized to 0~1
        else:
            raise ValueError(f'img ndim should be 3 or 4: {len(x.shape)}')
        x = torch.clip(x, 0, 1)  # clip to 0~1
        x = x.to(self.device)
        return x

    def prepare_videos(self, imgs: torch.Tensor) -> torch.Tensor:
        """ construct the input as standard
        imgs: NxBxHxWx3, uint8
        """
        assert len(imgs) == 1

        y = imgs[0].type(torch.float32) / 255.
        y = torch.clip(y, 0, 1)  # clip to 0~1
        y = y.unsqueeze(0)  # bsx3xHxW -> 1xbsx3xHxW
        y = y.to(self.device)
    ...
    def stitch(self, kp_source: torch.Tensor, kp_driving: torch.Tensor) -> torch.Tensor:
        ...
        """
        assert False, 'Using stitching module'
