...
class LivePortraitPipeline(nn.Module):
    ...
    def make_motion_template(self, I_lst, c_eyes_lst, c_lip_lst, **kwargs):
        ...
        for i in range(n_frames):
            ...
            c_eyes = c_eyes_lst[i].astype(np.float32) if c_eyes_lst is not None else None
            template_dct['c_eyes_lst'].append(c_eyes)

            c_lip = c_lip_lst[i].astype(np.float32) if c_lip_lst is not None else None
            template_dct['c_lip_lst'].append(c_lip)

        return template_dct

    def execute(self, src_t, src_x, tgt_x):
        # for convenience
        ...
        flag_load_from_template = is_template(args.driving) if args is not None else False
        driving_rgb_crop_256x256_lst = None
        wfp_template = None

        if args is None:
            flag_is_driving_video = False
            driving_rgb_lst = [tgt_x] # (bs, 3, 256, 256)
            output_fps = 25
            n_frames = len(source_rgb_lst)
            I_d_lst = self.live_portrait_wrapper.prepare_videos(driving_rgb_lst)
            driving_template_dct = self.make_motion_template(I_d_lst, None, None, output_fps=output_fps)
        ...
        # if args is not None and not flag_is_driving_video:
        ...
        if flag_is_source_video:
            ...
            if inf_cfg.flag_do_crop:
                ...
                source_lmk = None
                img_crop_256x256 = source_rgb_lst[0]
            ...
            # Nyte: Input retarget
            bs, _, _, c, h, w = src_t.shape
            f_s = self.live_portrait_wrapper.extract_feature_3d(src_t.reshape(bs, -1, h, w))
        ...
        for i in range(n_frames):
            ...
            # I_p_i = self.live_portrait_wrapper.parse_output(out['out'])
        ...
        if args is None:
            bs, c, h, w = out['out'].shape
            return out['out'].reshape(bs, 3, 1, -1, h, w) # (512, 512, 3)
        # elif flag_is_driving_video or (flag_is_source_video and not flag_is_driving_video):
