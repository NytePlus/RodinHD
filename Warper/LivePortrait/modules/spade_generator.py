...
class SPADEDecoder(nn.Module):
    def __init__(self, upscale=1, max_features=256, block_expansion=64, out_channels=64, num_down_blocks=2, output_channels=96):
        ...
        if self.upscale is None or self.upscale <= 1:
            self.conv_img = nn.Conv2d(out_channels, output_channels, 3, padding=1)
        else:
