    ...
    def run_cuda(self, triplane, rays_o, rays_d, dt_gamma=0, bg_color=None, perturb=False, force_all_rays=False, max_steps=1024, T_thresh=1e-4, **kwargs):
        ...
        if self.training:
            ...
            while step < max_steps:
                ...
                print(f'N: {N} n_step: {n_step} n_alive: {n_alive} den: {self.density_bitfield}')
                print(f'tri: {triplane.isnan().any()} rays_t: {rays_t.isnan().any()} rays_o: {rays_o.isnan().any()} rays_d: {rays_d.isnan().any()}')
